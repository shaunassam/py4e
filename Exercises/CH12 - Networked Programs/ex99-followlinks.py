...
# 
# Actual problem: Start at: http://py4e-data.dr-chuck.net/known_by_Leevi.html
...
repeatprocessnum = input("Enter count: ")
positionnum = input("Enter position: ")
repeatprocessnum = int(repeatprocessnum)
...
while True:
    ...
    for tag in tags:
        count += 1
        if count == positionnum:
            print(tag.get('href', None))
            url = str(tag.get('href', None))
            countrepeat += 1
            count = 0
            break
    if countrepeat == repeatprocessnum:
        break